...
const HeaderCell = ({ children, className }: { children: React.ReactNode; className?: string }) => (
  <th
    className={cn(
      "px-2 py-2 text-xs font-semibold uppercase tracking-tight",
      "text-[#6A7582]",
      className
    )}
  >
...
const DataCell = ({
  ...
  return (
    <td
      className={cn(
        "px-2 py-1.5 text-xs font-mono border-r border-slate-100 last:border-r-0",
        className
      )}
    >
      {displayVal}
...
export const OptionChainTable = ({
  ...
  const filteredRows = useMemo(() => {
    return rows.filter((row) => {
      ...
      const ceInRange =
        ceDelta !== null &&
        Math.abs(ceDelta) >= deltaRange.min &&
        Math.abs(ceDelta) <= deltaRange.max;
      const peInRange =
        peDelta !== null &&
        Math.abs(peDelta) >= deltaRange.min &&
        Math.abs(peDelta) <= deltaRange.max;
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-[#F9FBFE] border-b border-slate-200">
              <th
                colSpan={7}
                className="text-center py-2 text-xs font-bold text-emerald-600 border-r border-slate-200"
              >
                CALLS
              </th>
              <th className="py-2 bg-[#E8ECFC]"></th>
              <th colSpan={7} className="text-center py-2 text-xs font-bold text-red-600">
                PUTS
              </th>
            </tr>
            <tr className="bg-[#F9FBFE] border-b border-slate-200">
              ...
              {/* STRIKE */}
              <HeaderCell className="text-center bg-[#E8ECFC] text-[#212121] font-bold min-w-[80px]">
                Strike
              </HeaderCell>
          ...
          <tbody>
            {filteredRows.map((row) => {
              ...
              const ceDeltaVal =
                ce?.delta !== undefined && ce?.delta !== null ? Number(ce.delta) : null;
              const peDeltaVal =
                pe?.delta !== undefined && pe?.delta !== null ? Number(pe.delta) : null;
              ...
              const ceInRange =
                ceDeltaVal !== null &&
                Math.abs(ceDeltaVal) >= deltaRange.min &&
                Math.abs(ceDeltaVal) <= deltaRange.max;
              const peInRange =
                peDeltaVal !== null &&
                Math.abs(peDeltaVal) >= deltaRange.min &&
                Math.abs(peDeltaVal) <= deltaRange.max;
              ...
              return (
                <tr
                  key={row.strike}
                  className={cn(
                    "border-b border-slate-100 transition-colors hover:bg-slate-50/30",
                    borderClass
                  )}
                >
                  ...
                  <DataCell
                    ...
                    type="number"
                    precision={4}
                    className={cn("text-[#008D57]", callBg, ceOpacity)}
                  />
                  <DataCell
                    value={ce?.delta}
                    type="greek"
                    className={cn("text-[#212121]", callBg, ceOpacity)}
                  />
                  <DataCell
                    value={ce?.gamma}
                    type="greek"
                  ...
                  <DataCell
                    value={ce?.ltp}
                    type="currency"
                    className={cn("text-[#3F5BD9] font-semibold text-right", callBg, ceOpacity)}
                  />
                  ...
                  <DataCell
                    value={pe?.ltp}
                    type="currency"
                    className={cn("text-[#3F5BD9] font-semibold text-right", putBg, peOpacity)}
                  />
                  <DataCell
                    value={pe?.iv}
                    type="greek"
                    className={cn("text-[#212121]", putBg, peOpacity)}
                  />
                  <DataCell
                    value={pe?.vega}
                    type="greek"
                  ...
                  <DataCell
                    ...
                    type="number"
                    precision={4}
                    className={cn("text-[#008D57]", putBg, peOpacity)}
                  />
                </tr>
              );
            })}
