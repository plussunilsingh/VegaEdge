...
const PricingCard = ({ plan, onClick }: { plan: Plan; onClick: () => void }) => {
  return (
    ...
    >
      ...
      {/* Main Glass Body */}
      <div
        className="relative bg-slate-950/80 backdrop-blur-2xl rounded-[40px] p-8 border-2 border-white/10 shadow-2xl transition-all duration-1000 group-hover:rotate-y-0 group-hover:rotate-x-0 group-hover:translate-z-80 group-hover:-translate-y-12 preserve-3d overflow-hidden"
      ...
      >
        ...
        {plan.bestValue && (
          <div
            className="absolute top-8 right-8 bg-[#EAB308] text-black text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest shadow-[0_0_25px_rgba(234,179,8,0.5)] transition-transform duration-1000 group-hover:translate-z-100"
        ...
        <div className="relative preserve-3d">
          <div className="mb-8 transform transition-transform duration-1000 group-hover:translate-z-40">
            ...
            <div className="flex items-baseline gap-1" style={{ transform: "translateZ(60px)" }}>
              ...
              <span className="text-white/40 font-bold uppercase tracking-wider text-xs">
                {plan.duration}
              </span>
            </div>
          </div>
            ...
            {plan.features.map((feature, i) => (
              <div key={i} className="flex items-center gap-3 text-white/70">
                <div
                  ...
                  style={{ backgroundColor: plan.color, boxShadow: `0 0 10px ${plan.color}` }}
                />
...
const Subscription = () => {
  ...
  return (
    <section
      id="pricing"
      className="py-24 bg-background relative overflow-hidden text-foreground font-inter"
    >
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-20">
          <div className="flex justify-center items-center gap-2 mb-6">
            ...
            <h4 className="text-foreground text-2xl font-black tracking-tighter uppercase">
              Option <span className="text-primary">Greeks</span>{" "}
          ...
          <p className="text-muted-foreground text-lg font-medium">
            Unlock the power of deep volatility data and Greeks analysis for explosive trading
            performance.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12 perspective-container py-10 px-8">
          {plans.map((plan) => (
            <PricingCard key={plan.id} plan={plan} onClick={() => handlePlanClick(plan)} />
          ))}
        </div>

        <div className="mt-20 text-center opacity-40">
          <p className="text-xs font-bold tracking-[0.2em] uppercase">
            Trusted by 10,000+ Institutional Option Traders
          </p>
        </div>
      </div>
    </section>
