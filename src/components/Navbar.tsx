    ...
    <>
      ...
      >
        <div className="flex flex-col h-full overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-2">
            ...
            {/* Social Connectivity */}
            <div className="mt-8">
              <p className="text-slate-900 text-[10px] font-black uppercase tracking-widest px-4 mb-4">
                Connect With Us
              </p>
      ...
      <div className="bg-black text-white py-2 border-b border-white/10 relative z-[55]">
        <div className="container mx-auto px-4 lg:px-8 flex flex-col sm:flex-row justify-between items-center text-[10px] sm:text-xs font-medium tracking-wider gap-2">
          ...
          <div
            className={`flex items-center gap-2 font-bold ${isExpired ? "text-red-500 animate-pulse" : "text-primary"}`}
          >
            <Clock className="h-3 w-3" />{" "}
            {isExpired ? "Session Expired" : `Session: ${formatTime(sessionTimeLeft)}`}
          </div>
        </div>
      </div>
        ...
        <div className="container mx-auto px-4 lg:px-8">
          <div className="flex items-center justify-between h-16 sm:h-20">
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              >
                {isOpen ? (
                  <X className="h-7 w-7 sm:h-8 sm:w-8 text-primary" />
                ) : (
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              ...
              {!isAuthenticated ? (
                <Button
                  asChild
                  className="rounded-full px-5 sm:px-8 bg-primary hover:bg-primary/90 text-white font-bold h-9 sm:h-11 shadow-lg shadow-primary/20"
                >
                ...
                <div className="flex items-center gap-4">
                  <Link
                    to="/my-account"
                    className="flex items-center gap-2 hidden sm:inline-flex text-white/80 hover:text-white font-semibold text-sm border-r border-white/20 pr-4"
                  >
                    My Account{" "}
                  </Link>
                  <button
