...
const AdminDashboard = () => {
  ...
  const syncExpiriesMutation = useMutation({
    ...
    onSuccess: (data) =>
      toast.success(`Synced Expiries: ${Object.keys(data.data || {}).length} indices updated`),
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-card p-6 rounded-xl border border-border shadow-sm">
          <div>
            ...
            <p className="text-muted-foreground text-[11px] mt-1">
              Manage users, tokens, and system configuration.
            </p>
          </div>
        </div>
                        ...
                        <tbody className="divide-y divide-border">
                          {paginatedUsers.map((u: User) => (
                            <tr key={u.id} className="hover:bg-muted/30">
                                    ...
                                    <img
                                      src={
                                        u.profile_image.startsWith("http")
                                          ? u.profile_image
                                          : `${endpoints.auth.login.replace("/auth/login", "")}/${u.profile_image}`
                                      }
                                      alt=""
                                      className="w-8 h-8 rounded-full object-cover border border-border"
                                      onError={(e) =>
                                        (e.currentTarget.src = "/img/default_user.png")
                                      }
                                    />
                                  ) : (
                                    <img
                              ...
                              <td className="px-6 py-3">
                                <span
                                  className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${u.role === "ADMIN_USER" ? "bg-purple-500/10 text-purple-500" : "bg-muted text-muted-foreground"}`}
                                >
                                  {u.role}
                                </span>
                              </td>
                              ...
                              <td className="px-6 py-3 text-center">
                                <span
                                  className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${u.is_subscribed ? "bg-green-500/10 text-green-500" : "bg-red-500/10 text-red-500"}`}
                                >
          ...
          <TabsContent value="system" className="space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                ...
                >
                  <RefreshCw
                    className={`w-4 h-4 mr-2 ${refreshCacheMutation.isPending ? "animate-spin" : ""}`}
                  />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <Card>
                <CardHeader className="pb-3">
                  ...
                  <CardDescription className="text-xs">
                    Fetch latest contract expiries from Upstox to DB. (Runs daily at 08:30 AM
                    automatically)
                  </CardDescription>
                </CardHeader>
                <CardContent>
              ...
              <Card>
                <CardHeader className="pb-3">
                  ...
                  <CardDescription className="text-xs">
                    Login to Upstox to refresh access token.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <Button
              ...
              <Card>
                <CardHeader className="pb-3">
                  ...
                  <CardDescription className="text-xs">
                    Paste token if auto-generation fails.
                  </CardDescription>
                </CardHeader>
