...
const Strategies = () => {
  ...
  const strategies = {
    neutral: [
      { name: "Short Straddle", action: "Sell ATM Call + Sell ATM Put", legs: 2, risk: "Unlimited", bestWhen: "Low volatility" },
      { name: "Short Strangle", action: "Sell OTM Call + Sell OTM Put", legs: 2, risk: "Unlimited", bestWhen: "Range Bound" },
      { name: "Iron Butterfly", action: "Sell ATM Call + Put, Buy OTM Call + Put", legs: 4, risk: "Limited", bestWhen: "Earnings Reports" },
      { name: "Reverse Iron Condor", action: "Sell OTM Call + Put, Buy Further OTM Call + Put", legs: 4, risk: "Limited", bestWhen: "Wider Range" },
    ],
    event: [
      { name: "Long Straddle", action: "Buy ATM Call + Buy ATM Put", legs: 2, risk: "Limited", bestWhen: "Big Move Expected" },
      { name: "Long Strangle", action: "Buy OTM Call + Buy OTM Put", legs: 2, risk: "Limited", bestWhen: "Volatile events" },
      { name: "Reverse Iron Butterfly", action: "Buy ATM Call + Put, Sell OTM Call + Put", legs: 4, risk: "Limited", bestWhen: "Earnings Reports" },
       { name: "Reverse Iron Condor", action: "Buy ATM Call + Put, Sell OTM Call + Put", legs: 4, risk: "Limited", bestWhen: "FOMC Meetings" },
    ],
    bearish: [
       { name: "Bear Put Spread", action: "Buy ITM Put + Sell OTM Put", legs: 2, risk: "Limited", bestWhen: "Moderately Bearish" },
       { name: "Bear Call Spread", action: "Sell ITM Call + Buy OTM Call", legs: 2, risk: "Limited", bestWhen: "Moderately Bearish" },
    ],
    bullish: [
       { name: "Bull Call Spread", action: "Buy ITM Call + Sell OTM Call", legs: 2, risk: "Limited", bestWhen: "Moderately Bullish" },
       { name: "Bull Put Spread", action: "Sell ITM Put + Buy OTM Put", legs: 2, risk: "Limited", bestWhen: "Moderately Bullish" },
    ]
  };
      ...
      <div className="relative pt-32 pb-20 overflow-hidden">
        <div className="absolute inset-0 bg-primary/5 -z-10 transform -skew-y-2 origin-top-left scale-110"></div>
            ...
            <h1 className="text-4xl md:text-6xl font-black mb-6 tracking-tight">
                Global Market <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-blue-600">Strategies</span>
            </h1>
                    ...
                    <tbody className="divide-y divide-border">
                        {currentStrategies.map((strategy, index) => (
                            <tr key={index} className="hover:bg-muted/50 transition-colors">
                                ...
                                <td className="p-4">
                                    <span className={`px-2 py-1 rounded-full text-xs font-bold ${strategy.risk === 'Limited' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                        {strategy.risk}
                                    </span>
                                </td>
